...
## Solution 
- Trước tiên sau khi tải file ta được 1 file doc chứa macro
```
┌──(kali㉿kali)-[~/Downloads]
└─$ file emo.doc 
...
```
- Mình sẽ dùng olevba để xem macro nhưng có vẻ mã hơi khó đọc 
```
+----------+--------------------+---------------------------------------------+
...
+----------+--------------------+---------------------------------------------+
```
- Ta sẽ thực thi nó trên 1 môi trường ảo, ở đây chúng ta sử dụng any.run.
- ![image](image/1.PNG)
- Ta thấy rằng có 2 tiến trình đang chạy, theo dõi tiến trình powershell 
...
```
- Ta sử dụng cybercheff để decode ra, sau đó dùng 1 script nhỏ để tách từng lệnh ps 
```
