## Description 
> Với tư cách là nhà phân tích an ninh mạng thuộc Nhóm ứng phó sự cố của SecureTech, bạn đang giải quyết một vụ án khẩn cấp liên quan đến một tập đoàn nổi tiếng nghi ngờ có một cuộc tấn công mạng tinh vi vào mạng của họ. Tập đoàn quản lý dữ liệu quan trọng trong nhiều ngành khác nhau đã trải qua một cuộc tấn công bằng ransomware, dẫn đến việc mã hóa các tệp và cần có sự hỗ trợ ngay lập tức của chuyên gia để giảm thiểu thiệt hại và điều tra vi phạm.
Vai trò của bạn trong nhóm là tiến hành phân tích chi tiết các bằng chứng để xác định mức độ và tính chất của cuộc tấn công. Mục tiêu của bạn là xác định các chiến thuật, kỹ thuật và quy trình (TTP) được kẻ đe dọa sử dụng để giúp khách hàng của bạn ngăn chặn mối đe dọa và khôi phục tính toàn vẹn của mạng của họ.
## Link challenge 
> https://cyberdefenders.org/blueteam-ctf-challenges/bluesky-ransomware/
## Solution
- Mở file pcap lên để phân tích, ở đây ta sẽ phân tích bằng wireshark và NetworkMiner
- Trước tiên với wireshark, tại gói 2242 trở đi ta thấy có 1 giao thức TDS
```
Tabular Data Stream (TDS)  là một giao thức hoặc một bộ quy tắc mô tả cách truyền dữ liệu giữa hai máy tính. Giống như bất kỳ giao thức nào, nó xác định các loại tin nhắn có thể được gửi và thứ tự chúng có thể được gửi. Các giao thức mô tả "các bit trên dây", chỉ định cách truyền dữ liệu. Nó gây nhầm lẫn cho nhiều người nhưng lại dẫn đến một số ít.
```
- ![image](image/5.PNG)
...
- Tại NetworkMiner ta sẽ dễ dàng lấy được mật khẩu + username
- ![image](image/6.PNG)

- Hoặc ta cũng có thể lấy nó từ packet thứ 2641 trong wireshark 
- ![image](image/7.PNG)
- ![image](image/8.PNG)
...
- Từ câu trên ta cũng dễ dàng lấy được ip tấn công là 87.96.21.84
...
- Xem xét trong NetworkMiner, thấy attacker đã tải xuống 1 file tên `checking.ps1` với source code host là `87.96.21.84`
- ![image](image/10.PNG) 
...
- Mở lần lượt các file ps lên mình thấy được 1 file Invoke-PowerDump.ps1 chứa thông tin như sau 

<details>
<summary>
Xem file Invoke-PowerDump.ps1 tại đây 
</summary>
...
- Ngay từ những dòng đầu tiên ta thấy rằng lệnh powershell dump dữ liệu hash NTLM của thông tin mật khẩu người dùng
...
```
   ...
   .NOTES
     ...
     Privilage Escalation from http://blogs.technet.com/b/heyscriptingguy/archive/2012/07/05/use-powershell-to-duplicate-process-tokens-via-p-invoke.aspx
```
- Tại các hàm cũng là hàm để lấy dữ liệu như `function DecryptSingleHash($rid,[byte[]]$hbootkey,[byte[]]$enc_hash,[byte[]]$lmntstr)`, ` function DumpHashes`, ....

> Câu trả lời :  Invoke-PowerDump.ps1
